...
python build/build_memento.py all
...
```bash
python build/build_memento.py build
python build/build_memento.py install
```
...
```bash
python build/build_memento.py clean
```
...
Run the test suite:
```bash
python build/build_memento.py test
```
...
```bash
python build/build_memento.py check
```
...
```bash
python build/build_memento.py version
```
...
1. **"No distribution files found"**
   - Run `build` command first: `python build/build_memento.py build`
